...
登入球局後，點「開啟相機錄影」。App 會先保存目前房號、管理員權限與比賽規則，再開啟內建循環錄影。相機會從球局即時讀取目前兩隊球員與比分，疊在預覽並直接寫進保存的影片。影片每 10 秒暫存一段，只保留最近 18 段；按「保存最近 3 分鐘」時會先完成正在錄製的片段，再將最近片段寫入 `Movies/7B羽球`。錄影期間按鍵存取服務仍直接寫入同一份即時比分，不依賴背景中的網頁，因此疊圖、其他手機與 iPad 會一起更新，也不會調整音量或中斷錄影。

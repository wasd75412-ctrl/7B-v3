...
function openAdminNoticeEditor(){
  if(!isHost)return alert('只有管理員可以發布公告。');
  const old=state.adminNotice||{};
...
function closeAdminNoticeEditor(){$('adminNoticeModal').classList.add('hidden')}
async function publishAdminNotice(){
  ...
  state.adminNotice={title:title.slice(0,40),body:body.slice(0,500),publishedAt:new Date().toISOString()};
  renderDashboard();
  button.disabled=true;button.textContent='發布中…';
  try{
    await saveNow();
    closeAdminNoticeEditor();
    alert('事務公告已發布到所有裝置的總覽。');
  }catch(error){
    saveSoon();
    alert(`公告已保留，正在等待重新同步：${formatError(error)}`);
  }finally{
    button.disabled=false;button.textContent='發布公告';
  }
}

...
function renderPoll(){
  ...
  const unavailableRow=options.length&&!closed?`<label class="poll-option unavailable"><input class="viewer-enabled poll-choice poll-unavailable-choice" type="checkbox" value="${POLL_UNAVAILABLE}" ${mine.includes(POLL_UNAVAILABLE)?'checked':''}><div><strong>無法參加</strong>${isHost?`<div class="poll-voters">${unavailableVoters.length?`已選：${esc(unavailableVoters.join('、'))}`:'目前無人選擇'}</div>`:''}</div>${isHost?`<span class="poll-count">${unavailableCount} 人</span>`:''}</label>`:'';

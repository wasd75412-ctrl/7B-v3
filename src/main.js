...
function trimPollToConfirmedOption(poll){const optionId=state.nextEvent?.optionId,confirmed=(poll.options||[]).find(option=>option.id===optionId);if(!confirmed)return false;const needsTrim=(poll.options||[]).length!==1||Object.values(poll.votes||{}).some(value=>pollSelectionList(value).some(id=>id!==optionId));if(!needsTrim)return false;poll.options=[confirmed];const votes={},voterPlayers={};for(const [deviceHash,value] of Object.entries(poll.votes||{})){if(!pollSelectionList(value).includes(optionId))continue;votes[deviceHash]=optionId;if(poll.voterPlayers?.[deviceHash])voterPlayers[deviceHash]=poll.voterPlayers[deviceHash]}poll.votes=votes;poll.voterPlayers=voterPlayers;return true}
function renderPoll(){
  if(!$('pollOptions'))return;
  const poll=state.schedulePoll||{status:'open',deadlineAt:'',options:[],votes:{},voterPlayers:{}};
  const deadlineExpired=isPollDeadlinePassed(poll);if(deadlineExpired&&poll.status!=='closed'){poll.status='closed';if(isHost&&roomRef)setTimeout(()=>saveSoon(),0)}const closed=isPollClosed(poll),trimmed=closed&&trimPollToConfirmedOption(poll);if(trimmed&&isHost&&roomRef)setTimeout(()=>saveSoon(),0);const options=poll.options||[],counts=pollCounts(),mine=pollSelectionList(poll.votes?.[selfHash]),max=Math.max(0,...Object.values(counts));
...
function endPollEarly(){const poll=state.schedulePoll;if(isPollClosed(poll))return;if(!(poll.options||[]).length)return alert('目前沒有可結束的投票。');const optionId=state.nextEvent?.optionId,confirmed=(poll.options||[]).find(option=>option.id===optionId);if(!confirmed)return alert('請先在「確定下一次球局」選擇日期、填寫場地並發布到總覽，再結束投票。');const otherCount=poll.options.length-1,message=`將保留「${pollOptionLabel(confirmed)}」並結束投票。${otherCount?`\n\n其他 ${otherCount} 個候選日期會一併刪除。`:''}\n結束後球友將無法再修改投票。`;if(!confirm(message))return;poll.status='closed';trimPollToConfirmedOption(poll);renderPoll();renderDashboard();saveSoon();alert('投票已結束，其他候選選項已刪除。')}

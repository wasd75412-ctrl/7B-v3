...
function renderAndroidRemote(){
  ...
  const hasRecordingBridge=hasAndroidRecordingModeBridge();
...
async function writeNextEventPayment(eventId,playerId,status){
  if(!roomRef)return;
  const updatedEvents=await runTransaction(db,async tx=>{
    const snap=await tx.get(roomRef);if(!snap.exists())throw new Error('房間不存在');
    const decoded=decodeState(snap.data()),claimed=decoded.roster.find(item=>item.id===playerId&&playerOwnerHashes(item).includes(selfHash));
    if(!isHost&&!claimed)throw new Error('請先認領自己的球員資料。');
    const events=normalizeNextEvents(decoded),index=events.findIndex(event=>event.id===eventId);if(index<0)throw new Error('找不到這場球局。');
    events[index]=updateEventPayment(events[index],playerId,status,new Date().toISOString());
    const nextEvent=decoded.nextEvent&&nextEventIdentity(decoded.nextEvent)===eventId?events[index]:decoded.nextEvent;
    tx.update(roomRef,{nextEvents:events,nextEvent,updatedAt:serverTimestamp()});return events;
  });
  state.nextEvents=updatedEvents;const primary=primaryNextEvent({nextEvents:updatedEvents});if(primary)state.nextEvent=primary;renderNextEventAnnouncement();
}
async function reportNextEventPayment(eventId){
  const playerId=ownedPlayerId();if(!playerId)return alert('請先認領自己的球員資料。');
  const event=normalizeNextEvents(state).find(row=>row.id===eventId);if(!event)return;
  const status=eventPaymentStatus(event,playerId)==='pending'?'unpaid':'pending';
  try{await writeNextEventPayment(eventId,playerId,status)}catch(error){alert(formatError(error))}
}
async function moderateNextEventPayment(eventId,playerId,status){
  if(!isHost)return;
  try{await writeNextEventPayment(eventId,playerId,status)}catch(error){alert(formatError(error))}
}
...
function historicalPollChoices(){const today=taipeiDateKey();return state.pollHistory.slice().reverse().flatMap(poll=>(poll.options||[]).filter(option=>option.date>=today).map(option=>({value:`${poll.id}::${option.id}`,poll,option})))}
...
function renderDashboard() {
    ...
    if(activeTube){
      ...
      shuttleSummary.innerHTML=`<span><strong>🏸 ${esc(activeTube.name)}</strong><small>已使用 ${used} 顆 · 剩餘 ${activeTube.remainingShuttles} 顆</small></span><div class="home-shuttle-summary-actions"><strong class="home-shuttle-fee-amount">${players?`每人 ${formatMoney(share)} 元`:'等待完成比賽'}</strong>${isHost?'<button id="homeShuttleManagerBtn" class="btn" type="button">球桶管理</button>':''}</div>`;
    }else{
...
function renderPoll(){
  ...
  const activeEvent=primaryNextEvent();
  $('pollSetupPanel').style.display=closed?'none':'';$('pollVotingPanel').style.display=completed?'none':'';$('confirmEventPanel').style.display=!options.length?'none':'';$('pollCompletedPanel').classList.toggle('hidden',!completed);
  ...
  $('pollSummary').classList.toggle('hidden',!isHost);
  ...
  const dateRows=[...groupedOptions.entries()].map(([date,slots])=>{const slotRows=slots.sort((a,b)=>(a.time||'').localeCompare(b.time||'')).map(o=>{const participantIds=pollParticipantIds(o.id),count=counts[o.id]||0,full=count>=POLL_SLOT_CAPACITY,selectedByMe=mine.includes(o.id),voters=participantIds.map(pname).filter(n=>n!=='未知球員'),manualIds=cleanEventParticipantIds(poll.manualParticipants?.[o.id]),available=state.roster.filter(p=>!participantIds.includes(p.id)),durationRule=pollDurationRule(o.date,o.time),voterDetail=isHost?`<div class="poll-voters">${voters.length?`已選：${esc(voters.join('、'))}`:'尚無人選擇'} ${!closed?`<button type="button" class="btn danger-outline host-only poll-delete" data-poll-delete="${o.id}" style="padding:5px 8px;margin-left:6px">刪除</button>`:''}</div>`:'',manualBadges=manualIds.map(id=>`<button type="button" class="poll-manual-player" data-remove-manual-option="${esc(o.id)}" data-remove-manual-player="${esc(id)}">${esc(pname(id))} ×</button>`).join(''),manualControls=deadlineExpired&&isHost?`<div class="poll-manual-controls"><select class="input" data-manual-player="${esc(o.id)}" ${full?'disabled':''}><option value="">${full?'此時段已滿 6 人':'選擇要補加入的球員'}</option>${available.map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join('')}</select><button type="button" class="btn primary" data-add-manual-player="${esc(o.id)}" ${available.length&&!full?'':'disabled'}>＋ 加入球員</button></div>${manualBadges?`<div class="poll-manual-added">管理員加入：${manualBadges}</div>`:''}`:'';return `<label class="poll-option poll-time-option ${closed?'closed':''}"><input class="viewer-enabled poll-choice poll-date-choice" type="checkbox" value="${o.id}" ${selectedByMe?'checked':''} ${closed||full&&!selectedByMe?'disabled':''}><div>${durationRule?`<div class="poll-duration-rule">👥 ${esc(durationRule)}</div>`:''}${o.note?`<div class="poll-slot-venue"><span>📍 ${esc(o.note)}</span>${googleMapsLink(o.note,'查看場地')}</div>`:''}${voterDetail}${manualControls}</div><span class="poll-count">${count}/${POLL_SLOT_CAPACITY} 人${full?' · 額滿':''}</span></label>`}).join('');return `<section class="poll-date-group"><div class="poll-date-group-head"><strong>${esc(pollDateLabel(date))}</strong></div><div class="poll-time-options">${slotRows}</div></section>`}).join('');
  const unavailableVoters=isHost?Object.entries(poll.votes||{}).filter(([,v])=>pollSelectionList(v).includes(POLL_UNAVAILABLE)).map(([hash])=>pname(poll.voterPlayers?.[hash]||'')).filter(n=>n!=='未知球員'):[];
...
async function saveNextEventEdits(){
  ...
  const updatedEvent=cleanNextEvent({...previous,...(creating?{id:randomToken(),optionId:historyChoice?.option.id||''}:{}),date,time,endTime,location,note,participantIds,transferBankCode,transferAccount,rentalTotal,participantCount,perPersonFee,publishedAt});
...
async function confirmNextEvent(){
  if(!isHost)return alert('只有管理員可以建立球局。');
  const sourcePoll=confirmationPoll(),optionId=$('confirmPollOption').value,option=(sourcePoll.options||[]).find(o=>o.id===optionId),endTime=$('confirmEndTime').value,location=$('confirmLocation').value.trim(),note=$('confirmEventNote').value.trim(),transferBankCode=cleanTransferBankCode($('confirmTransferBankCode').value),transferAccount=cleanTransferAccount($('confirmTransferAccount').value);
  ...
  if(!confirm(`確定建立並發布球局？\n\n${summary}\n\n投票仍會持續到原訂截止時間。`))return;
  ...
  try{
    await runTransaction(db,async tx=>{
      ...
      if(pollWasFinalized(poll))throw new Error('這次投票已由另一台裝置發布完成。');
      ...
      const participantIds=pollParticipantIds(remoteOption.id,poll);
      ...
      tx.update(roomRef,{nextEvent:finalEvent,nextEvents,updatedAt:serverTimestamp()})
    });
...
async function submitPollVote(){
  ...
  try{
    await runTransaction(db,async tx=>{
      ...
      const proposedPoll={...poll,votes,voterPlayers,manualParticipants:cleanManualPollParticipants(poll.manualParticipants)};
      const fullOption=(Array.isArray(poll.options)?poll.options:[]).find(option=>validSelected.includes(option.id)&&pollParticipantCount(option.id,proposedPoll)>POLL_SLOT_CAPACITY);
      if(fullOption)throw new Error(`${pollOptionLabel(fullOption)} 已滿 6 人，請選擇其他時段。`);
      tx.update(roomRef,{schedulePoll:{status:poll.status||'open',createdAt:poll.createdAt||'',deadlineAt:poll.deadlineAt||'',options:Array.isArray(poll.options)?poll.options:[],votes,voterPlayers,manualParticipants:proposedPoll.manualParticipants},updatedAt:serverTimestamp()});
    });
...
async function syncDeviceProfileNow(){const code=deviceSyncCode(),identityToken=localStorage.getItem(DEVICE_SYNC_TOKEN_KEY)||'',ref=deviceProfileRef(code);if(!ref||!identityToken)return;clearTimeout(deviceProfileSaveTimer);deviceProfileSaveTimer=null;let merged=roomLibrary(),mergedVenues=favoriteVenues(),mergedTransfer=favoriteTransferDetails(),mergedPacking=loadEventPackingMemo();await runTransaction(db,async tx=>{const snapshot=await tx.get(ref),profile=snapshot.exists()?snapshot.data():{};if(profile.identityToken&&profile.identityToken!==identityToken)throw new Error('裝置同步身分不一致。');merged=mergeRoomLibraries(merged,profile.rooms);mergedVenues=mergeFavoriteVenues(mergedVenues,profile.favoriteVenues);mergedTransfer=mergedTransfer.transferAccount?mergedTransfer:cleanTransferDetails(profile.favoriteTransferDetails);if(!mergedPacking.updatedAt&&JSON.stringify(mergedPacking.items)!==JSON.stringify(EVENT_PACKING_MEMO_ITEMS))mergedPacking={...mergedPacking,updatedAt:Date.now()};mergedPacking=mergePackingMemos(mergedPacking,profile.packingMemo);const displayName=localStorage.getItem(DEVICE_SYNC_NAME_KEY)||profile.displayName||'潘建昱',playerName=localStorage.getItem(DEVICE_SYNC_PLAYER_KEY)||profile.playerName||displayName;tx.set(ref,{displayName,playerName,identityToken,rooms:merged,favoriteVenues:mergedVenues,favoriteTransferDetails:mergedTransfer,packingMemo:mergedPacking,updatedAt:serverTimestamp()},{merge:true})});applyCloudRoomLibrary(merged);applyCloudFavoriteVenues(mergedVenues);applyCloudFavoriteTransferDetails(mergedTransfer);applyCloudPackingMemo(mergedPacking)}
...
function applyCloudPackingMemo(value){const local=loadEventPackingMemo(),cloud=normalizeEventPackingMemo(value),merged=mergePackingMemos(local,cloud);saveEventPackingMemo(merged,{cloud:false});if(!$('eventPackingMemoModal').classList.contains('hidden'))renderEventPackingMemo();if(merged.updatedAt>local.updatedAt)void syncPackingReminderItems(merged);if(JSON.stringify(merged)!==JSON.stringify(cloud))queueDeviceProfileSave()}
async function initializeDeviceProfileSync(){updateDeviceSyncControls();const code=deviceSyncCode(),token=localStorage.getItem(DEVICE_SYNC_TOKEN_KEY)||'',ref=deviceProfileRef(code);if(!ref||!token)return;deviceProfileUnsubscribe?.();deviceProfileUnsubscribe=onSnapshot(ref,snapshot=>{if(!snapshot.exists())return;const profile=snapshot.data();if(profile.identityToken!==token)return;if(profile.displayName)localStorage.setItem(DEVICE_SYNC_NAME_KEY,profile.displayName);if(profile.playerName)localStorage.setItem(DEVICE_SYNC_PLAYER_KEY,profile.playerName);applyCloudRoomLibrary(profile.rooms);applyCloudFavoriteVenues(profile.favoriteVenues);applyCloudFavoriteTransferDetails(profile.favoriteTransferDetails);applyCloudPackingMemo(profile.packingMemo);updateDeviceSyncControls();ensureSyncedPlayerIdentity().catch(error=>console.warn('Player identity sync failed',error))},error=>console.warn('Device profile listener failed',error))}
async function setupDeviceSync(){
  ...
  if(input){
    ...
    applyCloudRoomLibrary(profile.rooms);applyCloudFavoriteVenues(profile.favoriteVenues);applyCloudFavoriteTransferDetails(profile.favoriteTransferDetails);applyCloudPackingMemo(profile.packingMemo);
...
function renderScore(){
  ...
  const servingLabel=`${m.serving===0?'A隊':'B隊'} · ${pname(sid)} · ${side}發球區`;
...
function openEdit(id){editId=id;const p=player(id);pendingAvatar=null;profileOriginal={name:p?.name||'',gender:normalizePlayerGender(p?.gender),memberType:normalizePlayerType(p?.memberType),voiceName:p?.voiceName||defaultVoiceName(p?.name),racket:p?.racket||'',racketTension:p?.racketTension||'',racketString:p?.racketString||'',backupRacket:p?.backupRacket||'',backupTension:p?.backupTension||'',backupString:p?.backupString||'',note:p?.note||''};profileDirty={name:false,gender:false,memberType:false,voiceName:false,racket:false,racketTension:false,racketString:false,backupRacket:false,backupTension:false,backupString:false,note:false};$('editName').value=profileOriginal.name;$('editGender').value=profileOriginal.gender;$('editMemberType').value=profileOriginal.memberType;$('editVoiceName').value=profileOriginal.voiceName;$('editRacket').value=profileOriginal.racket;$('editRacketTension').value=profileOriginal.racketTension;$('editRacketString').value=profileOriginal.racketString;$('editBackupRacket').value=profileOriginal.backupRacket;$('editBackupTension').value=profileOriginal.backupTension;$('editBackupString').value=profileOriginal.backupString;$('editNote').value=profileOriginal.note;refreshProfilePreview();updateProfilePermissions();setPlayerModalOpen(true)}
...
function sessionPlayedParticipantIds(dateKey=localDateKey()){return playedParticipantIds((state.history||[]).filter(match=>historyDate(match)===dateKey))}
function shuttleParticipantCount(){return sessionPlayedParticipantIds().length}
...
function sessionCombinedCosts(event=currentSessionEvent()){
  const tube=activeShuttleTube(),applies=!!event&&event.date===localDateKey(),used=currentSessionShuttleUsage(tube),shuttleTotal=used*shuttleUnitPrice(tube?.price),players=shuttleParticipantCount(),rentalTotal=wholeAmount(event?.rentalTotal),share=calculateCombinedPerPersonFee(rentalTotal,shuttleTotal,players);
...
function renderShuttleTubeManager(){
  ...
  const costPanel=tube=>{
    ...
    return `<div class="shuttle-cost-panel"><div><span>球桶價格 / 顆數</span><strong>${formatMoney(tube.price)} 元 / 12 顆</strong></div><div><span>每顆單價</span><strong>${formatMoney(shuttleUnitPrice(tube.price))} 元</strong></div><div><span>該場次已使用</span><strong>${used} 顆</strong></div><div><span>實際出賽人數</span><strong>${players} 人</strong><small>完成比賽即計入，退席後不扣除；購球者若參與也計入</small></div><div class="shuttle-share-total"><span>該場次每人分攤</span><strong>${players?`${formatMoney(share)} 元`:'等待完成比賽'}</strong></div></div>`;
  };
  const activeMarkup=active?`<article class="shuttle-tube-current">
    ...
    <div class="shuttle-stock ${active.remainingShuttles===0?'empty':''}">
      ...
      <div class="shuttle-stock-actions"><button class="btn primary" type="button" aria-label="使用一顆球" title="使用一顆球" data-shuttle-delta="-1" data-shuttle-tube="${esc(active.id)}" ${active.remainingShuttles===0?'disabled':''}>−1</button><button class="btn" type="button" aria-label="加回一顆球" title="加回一顆球" data-shuttle-delta="1" data-shuttle-tube="${esc(active.id)}" ${active.remainingShuttles>=active.totalShuttles?'disabled':''}>+1</button><button class="btn" type="button" data-set-shuttle-remaining="${esc(active.id)}">直接設定</button></div>
    </div>
    ${costPanel(active)}
...
let fastTap=null,fastTapDispatching=false,suppressNativeClick=null;
function fastTapButton(target){return target?.closest?.('button:not(:disabled)')||null}
document.addEventListener('pointerdown',event=>{
  if(!['touch','pen'].includes(event.pointerType)||event.button!==0)return;
  const button=fastTapButton(event.target);if(!button)return;
  fastTap={button,pointerId:event.pointerId,x:event.clientX,y:event.clientY,at:performance.now()};
},{capture:true,passive:true});
document.addEventListener('pointercancel',()=>{fastTap=null},{capture:true,passive:true});
document.addEventListener('pointerup',event=>{
  const tap=fastTap;fastTap=null;
  if(!tap||tap.pointerId!==event.pointerId||tap.button!==fastTapButton(event.target)||performance.now()-tap.at>650||Math.hypot(event.clientX-tap.x,event.clientY-tap.y)>12)return;
  suppressNativeClick={button:tap.button,until:performance.now()+700};
  fastTapDispatching=true;try{tap.button.click()}finally{fastTapDispatching=false}
},{capture:true,passive:true});
document.addEventListener('click',event=>{
  if(fastTapDispatching||!suppressNativeClick)return;
...
$('editGender').addEventListener('change',()=>profileDirty.gender=true);
...
$('randomCourt').onclick=()=>{
  ...
  state.court=needed===2?players:teammateSafeLineup(players,{randomize:true,randomizeAll:true});

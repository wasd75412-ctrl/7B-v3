...
import { arrangeLineupByGender, normalizeGender } from './lineup.js';
import { normalizeYouTubePlaylistUrl } from './youtube.js';
...
function decodeState(d){
  ...
  return {
    ...
    roster:Array.isArray(d.roster)?d.roster.map(p=>({...p,gender:normalizeGender(p.gender),voiceName:p.voiceName||defaultVoiceName(p.name),backupRacket:p.backupRacket||'',racketTension:p.racketTension||'',racketString:p.racketString||'',backupTension:p.backupTension||'',backupString:p.backupString||'',favorite:!!p.favorite})):[],
...
async function confirmNextEvent(){
  ...
  try{
    await runTransaction(db,async tx=>{
      ...
      finalFee=Math.ceil(rentalTotal/remoteParticipants.size);
...
function genderAwareLineup(ids,{randomize=false}={}){
  const selected=uniqueIds(ids).slice(0,4);
  const base=randomize?shuffle(selected):selected;
  return arrangeLineupByGender(base,id=>player(id)?.gender);
}
...
function renderMatchReplay(){
  const url=normalizeYouTubePlaylistUrl(state.matchReplayPlaylistUrl),card=$('matchReplayCard'),link=$('matchReplayLink'),input=$('matchReplayUrl'),clear=$('clearMatchReplay');
...
async function saveMatchReplayPlaylist(){
  if(!isHost)return;
  const input=$('matchReplayUrl'),button=$('saveMatchReplay'),url=normalizeYouTubePlaylistUrl(input.value);
  if(!url)return setMatchReplayFeedback('請貼上正確的 YouTube 播放清單網址。','error');
  const previous=state.matchReplayPlaylistUrl;
  ...
  try{
    ...
    state.matchReplayPlaylistUrl=previous;
    renderMatchReplay();
...
async function clearMatchReplayPlaylist(){
  ...
  const previous=state.matchReplayPlaylistUrl,button=$('clearMatchReplay');
...
function startMatch(){dismissedResultKey='';const selected=state.court.filter(Boolean);if(selected.length!==4||new Set(selected).size!==4)return alert('請選擇四位不同球員。');const ids=genderAwareLineup(selected);state.court=[...ids];reconcileWaitingQueue(ids);state.queueDraftChosen=[];randomizeScoreThemeAtMatchStart();state.match={active:true,players:[[ids[0],ids[1]],[ids[2],ids[3]]],scores:[0,0],rallies:[],serving:0,positions:[[0,1],[0,1]],winner:null,startedAt:new Date().toISOString()};saveSoon();renderScore();renderDashboard()}
function finishMatch(){
  ...
  state.priority=state.waitingQueue[0]||null;
  const four=genderAwareLineup([...winners,...chosen],{randomize:true});
...
function startNext(){
  ...
  const vals=genderAwareLineup(selected);
...
function updateProfilePermissions(){const p=player(editId),editable=canEditPlayer(p),claimable=!isHost&&p&&!editable;const claimBtn=$('claimPlayer');claimBtn.classList.toggle('hidden',!claimable);claimBtn.textContent=p?.ownerHash?'這是我的資料／重新認領':'這是我／認領資料';$('saveEdit').classList.toggle('hidden',!editable);$('profileEditFields').classList.toggle('hidden',!editable);$('photoHint').classList.toggle('hidden',!editable);['editName','editGender','editRacket','editRacketTension','editRacketString','editBackupRacket','editBackupTension','editBackupString','editNote','editPhoto','removePhoto'].forEach(id=>{const el=$(id);if(el)el.disabled=!editable});const voiceSection=$('voiceAdminSection');if(voiceSection)voiceSection.classList.toggle('hidden',!isHost);const voiceInput=$('editVoiceName');if(voiceInput)voiceInput.disabled=!isHost;const testVoice=$('testVoiceName');if(testVoice)testVoice.disabled=!isHost}
function openEdit(id){editId=id;const p=player(id);pendingAvatar=null;profileOriginal={name:p?.name||'',gender:normalizeGender(p?.gender),voiceName:p?.voiceName||defaultVoiceName(p?.name),racket:p?.racket||'',racketTension:p?.racketTension||'',racketString:p?.racketString||'',backupRacket:p?.backupRacket||'',backupTension:p?.backupTension||'',backupString:p?.backupString||'',note:p?.note||''};profileDirty={name:false,gender:false,voiceName:false,racket:false,racketTension:false,racketString:false,backupRacket:false,backupTension:false,backupString:false,note:false};$('editName').value=profileOriginal.name;$('editGender').value=profileOriginal.gender;$('editVoiceName').value=profileOriginal.voiceName;$('editRacket').value=profileOriginal.racket;$('editRacketTension').value=profileOriginal.racketTension;$('editRacketString').value=profileOriginal.racketString;$('editBackupRacket').value=profileOriginal.backupRacket;$('editBackupTension').value=profileOriginal.backupTension;$('editBackupString').value=profileOriginal.backupString;$('editNote').value=profileOriginal.note;refreshProfilePreview();updateProfilePermissions();$('editModal').classList.remove('hidden')}
...
async function addPlayerRecord(){
  ...
  const syncedName=(localStorage.getItem(DEVICE_SYNC_PLAYER_KEY)||localStorage.getItem(DEVICE_SYNC_NAME_KEY)||'').trim(),record={id:randomToken(),name,gender:'',voiceName:defaultVoiceName(name),avatar:'',racket:'',racketTension:'',racketString:'',backupRacket:'',backupTension:'',backupString:'',note:'',favorite:false,ownerHash:name===syncedName?selfHash:''};
  button.disabled=true;
  try{
...
$('goCourt').onclick=()=>{
  ...
  state.court=genderAwareLineup(state.court.length>=4?state.court:state.attendance.slice(0,4));
  reconcileWaitingQueue(state.court);renderAll();page(3);saveSoon();
};
$('randomCourt').onclick=()=>{
  state.court=genderAwareLineup(shuffle(state.attendance).slice(0,4));
  reconcileWaitingQueue(state.court);renderAll();saveSoon();
};
$('shuffleNext').onclick=()=>{
  const vals=genderAwareLineup([0,1,2,3].map(i=>$('n'+i).value),{randomize:true});

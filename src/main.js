...
function renderPoll(){
  ...
  const dateRows=[...groupedOptions.entries()].map(([date,slots])=>{const commonVenue=slots.find(option=>option.note)?.note||'';const slotRows=slots.sort((a,b)=>(a.time||'').localeCompare(b.time||'')).map(o=>{const voters=Object.entries(poll.votes||{}).filter(([,v])=>pollSelectionList(v).includes(o.id)).map(([hash])=>pname(poll.voterPlayers?.[hash]||'')).filter(n=>n!=='未知球員');return `<label class="poll-option poll-time-option ${closed?'closed':''}"><input class="viewer-enabled poll-choice poll-date-choice" type="checkbox" value="${o.id}" ${mine.includes(o.id)?'checked':''} ${closed?'disabled':''}><div><strong>${esc(`${o.time||'未設定'}${o.endTime?`–${o.endTime}`:''}`)}</strong><div class="poll-voters">${voters.length?`已選：${esc(voters.join('、'))}`:'尚無人選擇'} ${isHost&&!closed?`<button type="button" class="btn danger-outline host-only poll-delete" data-poll-delete="${o.id}" style="padding:5px 8px;margin-left:6px">刪除</button>`:''}</div></div><span class="poll-count">${counts[o.id]||0} 票</span></label>`}).join('');return `<section class="poll-date-group"><div class="poll-date-group-head"><strong>${esc(pollDateLabel(date))}</strong>${commonVenue?`<span>${esc(commonVenue)}</span>${googleMapsLink(commonVenue,'查看場地')}`:''}</div><div class="poll-time-options">${slotRows}</div></section>`}).join('');

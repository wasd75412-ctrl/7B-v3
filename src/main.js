...
function genderSymbol(value){return normalizeGender(value)==='male'?'♂':normalizeGender(value)==='female'?'♀':''}
function genderLabel(value){return normalizeGender(value)==='male'?'男性':normalizeGender(value)==='female'?'女性':'性別未設定'}
...
function options(selected=''){return `<option value="">請選擇</option>`+state.attendance.map(id=>{const gender=normalizeGender(player(id)?.gender),symbol=genderSymbol(gender);return `<option value="${id}" ${id===selected?'selected':''}>${gender?`${symbol} ${genderLabel(gender)} · `:''}${esc(pname(id))}</option>`}).join('')}

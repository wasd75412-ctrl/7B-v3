...
function renderAdminAnnouncement(){
  ...
  if($('editAdminNoticeBtn'))$('editAdminNoticeBtn').disabled=!notices.length;
  if($('clearAdminNoticeBtn'))$('clearAdminNoticeBtn').disabled=!notices.length;
  ...
  box.innerHTML=notices.map(notice=>{
    ...
    const actions=isHost?`<div class="admin-notice-entry-actions"><button class="btn" type="button" data-edit-admin-notice="${esc(notice.id)}">編輯</button><button class="btn danger-outline" type="button" data-delete-admin-notice="${esc(notice.id)}">刪除</button></div>`:'';
    return `<article class="admin-notice-entry"><div class="admin-notice-entry-head"><h3>📣 ${esc(notice.title)}</h3>${actions}</div><p>${esc(notice.body)}</p>${time?`<time>發布於 ${esc(time)}</time>`:''}</article>`;
  }).join('');
  all('[data-edit-admin-notice]').forEach(button=>button.onclick=()=>openAdminNoticeEditor(button.dataset.editAdminNotice));
...
function openAdminNoticeEditor(id=''){
  if(!isHost)return alert('只有管理員可以發布公告。');
  const old=id?normalizeAdminNotices(state).find(notice=>notice.id===id):null;
  if(id&&!old)return alert('找不到這則公告，可能已由其他裝置刪除。');
  editingAdminNoticeId=old?.id||'';
  $('adminNoticeEditorTitle').textContent=old?'✏️ 編輯公告':'📣 新增公告';
  $('adminNoticeTitle').value=old?.title||'事務通知';
  $('adminNoticeBody').value=old?.body||'';
  $('saveAdminNotice').textContent=old?'儲存修改':'新增公告';
  $('adminNoticeModal').classList.remove('hidden');
  setTimeout(()=>$('adminNoticeTitle').focus(),50);
}
function closeAdminNoticeEditor(){editingAdminNoticeId='';$('adminNoticeModal').classList.add('hidden')}
async function publishAdminNotice(){
  ...
  const notices=normalizeAdminNotices(state),publishedAt=new Date().toISOString(),wasEditing=!!editingAdminNoticeId,record={id:editingAdminNoticeId||randomToken(),title:title.slice(0,40),body:body.slice(0,500),publishedAt};
  if(wasEditing&&!notices.some(notice=>notice.id===editingAdminNoticeId))return alert('這則公告已由其他裝置刪除，請關閉後重新新增。');
  setAdminNotices(editingAdminNoticeId?notices.map(notice=>notice.id===editingAdminNoticeId?record:notice):[record,...notices]);
  renderDashboard();
  button.disabled=true;button.textContent=wasEditing?'儲存中…':'新增中…';
  try{
    await saveNow();
    closeAdminNoticeEditor();
    alert(wasEditing?'公告已修改並同步到所有裝置。':'公告已新增並同步到所有裝置。');
  }catch(error){
    saveSoon();
    alert(`公告已保留，正在等待重新同步：${formatError(error)}`);
  }finally{
    button.disabled=false;button.textContent=editingAdminNoticeId?'儲存修改':'新增公告';
  }
}
async function deleteAdminNotice(id=''){
  ...
  setAdminNotices(notices.filter(notice=>notice.id!==target.id));renderDashboard();
  try{await saveNow();alert('公告已刪除。')}catch(error){setAdminNotices(before);renderDashboard();alert(`刪除失敗，公告已保留：${formatError(error)}`)}
}
async function clearAdminNotice(){return deleteAdminNotice()}
...
$('publishAdminNoticeBtn').onclick=()=>openAdminNoticeEditor();
$('editAdminNoticeBtn').onclick=()=>openAdminNoticeEditor(normalizeAdminNotices(state)[0]?.id||'');
$('saveAdminNotice').onclick=publishAdminNotice;
$('cancelAdminNotice').onclick=closeAdminNoticeEditor;
$('adminNoticeBody').addEventListener('keydown',event=>{if((event.ctrlKey||event.metaKey)&&event.key==='Enter')publishAdminNotice()});
$('clearAdminNoticeBtn').onclick=clearAdminNotice;
